...
RunAction::RunAction(SaveToFileEventAction* eventAction)
  ...
  // Creating histograms
  G4double maxEnergy = 50 * GeV;
  G4double maxLong = 60 * 4.45 * mm;
  G4double maxTrans = 98 * mm;
  G4int numLong = 60;
  G4int numTrans = 1000;
  ...
  analysisManager->CreateH1("Time", "Simulation time; time (s);Normalised entries", 3000, 0, 30);
  analysisManager->CreateH1("EnergyRatio", "Ratio of deposited to primary energy;E_{dep} /  E_{MC};Normalised entries", 100, 0, 1);
  analysisManager->CreateH1("longFirstMoment", "Longitudinal first moment;#LT#lambda#GT (ID);Normalised entries", 100, 0, maxLong);
  analysisManager->CreateH1("transFirstMoment", "Transverse first moment;#LTr#GT (ID);Normalised entries", 100, 0, maxTrans/10.);
  analysisManager->CreateH1("longSecondMoment", "Longitudinal second moment;#LT#lambda^{2}#GT (ID^{2});Normalised entries", 100, 0, pow(maxLong,2)/10);
  analysisManager->CreateH1("transSecondMoment", "Transverse second moment;#LTr^{2}#GT (ID^{2});Normalised entries", 100, 0, pow(maxTrans/10.,2));
