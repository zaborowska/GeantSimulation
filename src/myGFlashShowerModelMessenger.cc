...
{ 
  ...
  EmaxCmd = new G4UIcmdWithADoubleAndUnit("/myGFlash/Emax",this);
  ...
  EkillCmd = new G4UIcmdWithADoubleAndUnit("/myGFlash/Ekill",this);
...
myGFlashShowerModelMessenger::~myGFlashShowerModelMessenger()
{
  delete ContCmd;
  delete FlagCmd;
  delete StepInX0Cmd;  
...
void myGFlashShowerModelMessenger::
...
G4String myGFlashShowerModelMessenger::GetCurrentValue(G4UIcommand * command)
{
  G4String returnValue('\0');
  std::ostringstream os;
  
  if( command == FlagCmd ) { 
    os << "/myGFlash/flag " << myModel->GetFlagParamType()  << '\0';
